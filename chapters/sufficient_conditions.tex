...
To find separate bounds on the wave speed and growth rate, we need to somehow split the Orr-Sommerfeld equation in a way that gets us two separate equations, one for the wave speed and one for the growth rate. If we multiply the Orr-Sommerfeld equation \eqref{oseq} by $ \phi_c $ (the complex conjugate of $ \phi $) and integrating with respect to $ z \in (-1, 1) $ we obtain the following equation:
\begin{gather}
...
Throughout this chapter, we shall use the same notation as \citet{drazin2004hydrodynamic} to represent some commonly occurring integrals
\begin{align} 
...
These equations have the same form as derived by \citet{pai1954generalization}, though Pai used a generalised set of boundary conditions, whereas we have used our specific boundary conditions \eqref{osbc}.

We now have equations for both the wave speed $ c_r $ and the growth rate $ c_i $. This chapter will now turn to using these to get bounds on both.
...
If we look at the integrals in the above \eqref{paibeginning} we can see that there are a few different possible cases. First if $ c_r < 0$, then depending on the sign of $ U $ there are two possibilities. Since it is a non-trivial function, $ U $ can be split into regions where it is strictly positive and strictly negative. 

For ranges of $ z $ where $ U $ is strictly positive, $ U - c_r > 0 $, implying that the first integral must be positive which means that the second integral must be negative. This means that the statement inside the second integral must also be negative, implying that
\begin{gather}
    (U - c_r) + \frac{U''}{2 \al^2} < 0 \text{ for some } z \in (-1, 1) \nonumber\\
    \implies c_r > \frac{U''_{min}}{2 \al^2} + U_{min}, 
    \label{inequality}
\end{gather}
where the $min$ subscript indicates the minimum value taken by $U$ or $U''$ on this region.

For ranges of $z $ where $ U $ is strictly negative, $ U - c_r < 0 $. implying that the first integral must be negative and therefore the second integral must be positive. This gives a similar result, that 
\begin{gather}
    (U - c_r) + \frac{U''}{2 \al^2} > 0 \text{ for some } z \in (-1, 1) \nonumber\\
    \implies c_r > \frac{U''_{max}}{2 \al^2} + U_{max},
\end{gather}
where similarly, the $max$ subscript indicates the maximum value taken by $U$ or $U''$ on this region.

The second case is the case where $ c_r > 0 $. In a region where $ U $ is strictly positive, we need that the first integral is negative to get a different inequality (and therefore an upper bound) to \eqref{inequality}. We, therefore, impose the additional condition that $ c_r \geq U_{max} $, where $ U_{max} $ is the maximum value taken by $U $ on this region. The first integral is now negative which means that the second integral must be positive. This means that
\begin{gather}
    (U - c_r) + \frac{U''}{2 \al^2} > 0 \text{ for some } z \in (-1, 1) \nonumber\\
    \implies c_r < \frac{U''_{max}}{2 \al^2} + U_{max}. 
\end{gather}

In a region where $ U $ is negative, we want the first integral to be positive, meaning that we need the additional condition $ c_r < U_{min}$, where $U_{min} $ is the minimum value taken by $ U $ on this region. The first integral is now positive, implying that the second integral is negative. This implies that
\begin{gather}
    (U - c_r) + \frac{U''}{2 \al^2} < 0 \text{ for some } z \in (-1, 1) \nonumber\\
    \implies c_r > \frac{U''_{min}}{2 \al^2} + U_{min}. 
\end{gather}

Combining these inequalities gives the same bounds for both the strictly positive and strictly negative cases 
\begin{equation}
    \frac{U''_{min}}{2 \al^2} + U_{min} < c_r < \frac{U''_{max}}{2 \al^2} + U_{max}.
    \label{paicr}
\end{equation}

If we now expand across all the regions of positive and negative $ U $, we notice that the $ max$ and $ min$ subscripts can safely be redefined to be the maximum and minimum values taken by $ U$ and $ U''$ across the whole range of $ z = -1 $ to $ z = 1$. This bound was first derived by \citet{pai1954generalization}, though he used generalised bounds and left out the "$ U $ is strictly negative" case, presumably because of the argument applied here.
...
We can improve this result by considering the mean values of $ U $ and $ U'' $. Let $ a $ and $ b $ represent the mean values of $ U $ and $ U'' $ respectively (in the range $ (-1, 1) $). From \eqref{c_r} we have that 
\begin{align}
    c_r &= \frac{(U(a) + \al^2 U(a)) \I{0} + \tfrac{1}{2} U''(b) \I{0}}{\I{1} + \al^2 \I{0}} \nonumber\\
    &= \frac{(U(a) + \al^2 U(a)) \I{0} + \tfrac{1}{2} U''(b) \I{0}}{\I{1} + \al^2 \I{0}} \nonumber\\
    &= U(a) + \frac{U''(b)}{2(\tfrac{\I{1}}{\I{0}} + \al^2)}. \label{thm2cr}
\end{align}  

Again there are multiple cases. Let us start with the case that $ U''$ is strictly positive. If $ U''_{min} \geq 0 $, we must have that $ U''_{max} > 0 $ and therefore that
\begin{equation}
    0 < \frac{U''(b)}{2\qb{\tfrac{\I{1}}{\I{0}} + \al^2}},
\end{equation}
since $ U_{min} < U(a) < U_{max} $ by definition. If we now use the fact that $ \frac{\I{1}}{\I{0}} \geq \tfrac{1}{4} \pi^2 $ from our inequalities on $ \I{i}$ \eqref{lamineq}, we end up with 
\begin{equation}
    0 < \frac{2 U''_{max}}{\pi^2 + 4 \al^2}. 
\end{equation}

This leaves us with the bound 
\begin{equation}
    U_{min} < c_r < U_{max} + \frac{2 U''_{max}}{\pi^2 + 4 \al^2}, \label{crbound1}
\end{equation}
for this case. Note that the lower bound cannot be improved since we don't know whether $ U''_{min}$ is at the same value of $z$. 

If we consider a case where $ U'' $ is strictly negative, we must have that $ U''_{max} \leq 0 $. This implies that  
\begin{equation}
    \frac{2 U''_{min}}{\pi^2 + 4 \al^2} < \frac{U''(b)}{2 \qb{\tfrac{\I{1}}{\I{0}} + \al^2}},
\end{equation}
leaving us with the following bounds in this strictly negative case
\begin{equation}
     U_{min} + \frac{2 U''_{min}}{\pi^2 + 4 \al^2} < c_r < U_{max}. \label{crbound2}
\end{equation}

There is one final case to consider, the case where $ U''$ is not strictly positive or negative. In this case, we have that $ U''_{min} < 0 < U''_{max} $. This means we have the following bounds on the $ U'' $ term in \eqref{thm2cr}, 
\begin{equation}
    \frac{U''_{min}}{2 \qb{\tfrac{1}{4} \pi^2 + \al^2}} < \frac{U''(b)}{2 \qb{\tfrac{\I{1}}{\I{0}} + \al^2}} < \frac{U''_{max}}{2 \qb{\tfrac{1}{4} \pi^2 + \al^2}}. 
\end{equation}

This leaves us with our final bound on $ c_r$,
\begin{equation}
    U_{min} + \frac{2 U''_{min}}{\pi^2 + 4 \al^2}< c_r < \frac{2 U''_{max}}{\pi^2 + 4 \al^2}. \label{crbound13}
\end{equation}

Combining these three bounds \eqref{crbound1}, \eqref{crbound2} and \eqref{crbound13} gives us the following theorem first derived by \citet{joseph1969eigenvalue}.
...
For Poiseuille flow between $ -1 < z < 1 $, we have that $ U'' = -2 $, which means that case 3 of the theorem is satisfied. This leaves us with the following bounds for $ c_r $:
\begin{equation}
    \frac{-4}{\pi^2 + 4 \al^2} < c_r < 1.
\end{equation}

In the case of Couette flow, we have that $ U'' = 0 $, which means that cases 1 and 3 of the theorem are satisfied. This may seem like an issue, but they both give the same bounds on $ c_r$, 
\begin{equation}
    -1 < c_r < 1.
\end{equation}
...
The growth rate is important because a state cannot be stable if $ c_i > 0 $ for any eigenvalue of \eqref{oseq}. This is because if we look at our streamfunction solution \eqref{streamfunction} we can see that any solutions with $ c_i > 0 $ will blow up as $ t $ increases, while any with $ c_i < 0$ will decay, implying they are stable. $ c_i > 0 $ is therefore generally taken as the \emph{criterion for instability}.
...
To improve this bound on $ c_i $, consider a few more results as in \citet{joseph1968eigenvalue, joseph1969eigenvalue}. These all use \eqref{lamineq}. First we show that $ 2 \al I_0 I_1 \leq \I{1} + \al^2 \I{0} $
\begin{gather}
...
The second result we need is to show that $ \frac{\I{2}}{I_0 I_1} \geq \pi (\frac{4.73}{2})^2 $.
\begin{gather}
...
\begin{gather}
    ...
    c_i \leq \frac{q}{2 \al } - (\al R)^{-1} \qb{\frac{\pi^2 (\pi^2 + \al^2)}{\pi^2 + 4 \al^2} + \al^2}. \label{josephpart1}
\end{gather}
This is the first result of Joseph's Theorem 1 \citep{joseph1969eigenvalue}.

We'd like to find bounds on $ \al $ and $ R $ to see when a flow is stable. Consider a stable flow so that $ c_i < 0 $. If we rearrange \eqref{c_iineq}, we find that the following inequality holds
\begin{equation}
    \frac{(q I_1 I_0 - (\alpha R)^{-1} (\I{2} + 2 \alpha^2 \I{1} + \alpha^4 \I{0}))}{\I{1} + \alpha^2 \I{0}} < 0.
\label{stabineq}
\end{equation}

It would be nice to get a bound on stability that doesn't require us to have a solution $ \p$ and just depends on $\al $ and $R$, similar to \eqref{josephpart1}. Let's start with a less accurate method by considering the following integral as in \citet{synge1938hydrodynamical}
\begin{align}
...
From here, we select 3 different pairs of $ a, b $ as given in \citet{synge1938hydrodynamical}.
...
The names $ A_1, A_2, A_3 $ come from \citet{synge1938hydrodynamical}. We now have the result that a flow is definitely stable if $ \al R q < \max[A_1, A_2, A_3]$. This result was compiled into the following theorem by \citet{joseph1968eigenvalue}.
...
To improve Synge's result, rearrange \eqref{stabineq} to find that the following inequality must be satisfied by a stable flow as found by \citet{joseph1968eigenvalue}
\begin{equation}
    \al R q < \frac{\I{2} + 2 \al^2 \I{1} + \al^4 \I{0}}{I_1 I_0}.
\label{jaRq}
\end{equation}

Now rearrange the right-hand side of the above \eqref{jaRq} to get
\begin{gather}
    \frac{\I{2} + 2 \al^2 \I{1} + \al^4 \I{0}}{I_0 I_1} = \frac{\I{2}}{I_1 I_0} + \frac{2 \al^2 \I{1} + \al^4 \I{0}}{I_1 I_0} \nonumber\\
    \geq \tfrac{1}{2} \pi \qb{\frac{4.73}{2}}^2 + \frac{2 \al^2 \I{1} + \al^4 \I{0}}{I_0 I_1}.
\end{gather}
...
and that 
\begin{equation}
    \frac{2 \al^2 \I{1} + \al^4 \I{0}}{I_0 I_1} = \frac{2 \al^2}{I_0 I_1} \qb{\qb{I_1 - \frac{\al}{\sqrt{2}} I_0}^2 + \tfrac{2 \al}{\sqrt{2}} I_0 I_1}.
    \label{jthm1eqn4}
\end{equation}
...
\begin{align}
    ...
    \begin{cases}
        \frac{2 \al^2}{I_0 I_1} \qb{\qb{I_1 - \frac{\al}{\sqrt{2}} I_0}^2 + \tfrac{2 \al}{\sqrt{2}} I_0 I_1}, \\
...
This time the names $ M_1,\ M_2 $ come from \citet{joseph1968eigenvalue}. This gives the result that 
\begin{equation}
    \al R q < \max[M_1, M_2]. \label{josephpart2}
\end{equation}

The result \eqref{josephpart1} from earlier combined with this second result \eqref{josephpart2} forms the following theorem from \cite{joseph1968eigenvalue}.
...
where
\begin{align}
    M_1 &= \qb{\frac{4.73}{2}}^2 \pi + 2^{\tfrac{3}{2}} \al^3, \nonumber\\
    M_2 &= \qb{\qb{\frac{4.73}{2}}^2 + \al^2} \pi.
    \label{jbound}
\end{align}
\end{theorem}

Figure \ref{fig:josephvsynge} shows a comparison between Synge's bounds \eqref{sbounds} and Joseph's bounds \eqref{jbound}. The lines were plotted by considering the point at which the bounds are equal. The orange line represents the bounds given by Synge, while the blue line represents the bounds given by Joseph. The area below each curve represents the areas where we would expect no instability, while the area above the curve is where would expect instability.  As such, these are referred to as \emph{curves of marginal stability}. The area below both curves is referred to as the \emph{area of certain linear stability} \citep{joseph1968eigenvalue}.
