...
A plane flow is a 2-dimensional flow represented by a velocity profile $ U $ in a single direction. For this project, we will consider $ U $ that are functions of $ z $ only and act in the $ x $ direction. Moreover, our flow will be considered to be flowing through a 'channel' that is bounded between $ z = z_{1} $ and $ z = z_{1} $.

We denote our basic steady flow as
\begin{equation}
    \mathbf{U} = U (z) \mathbf{i} \hspace{10px} (z_{1} \leq z_* \leq z_{2}),
\end{equation}
with boundaries on $z = z_{1} \text{and } z_{2}$ that are either \emph{rigid} or \emph{free}:
\begin{itemize}
...
\begin{align}
    t_* = \frac{t V}{L},\ \mathbf{x_*} &= \frac{\mathbf{x}}{L},\ \mathbf{u_*} = \frac{\mathbf{u}}{V},\ p_* = \frac{p}{\rho V^2}, \nonumber\\ \mathbf{U_*} &= \frac{\mathbf{U}}{V} \equiv U_*(z_*) \mathbf{i},
    \label{coord_trans}
\end{align}
where we refer to $ V $ as the \emph{characteristic velocity} and $ L $ as the \emph{characteristic length}, defined as
\begin{align}
    V &= \text{max}_{z \in [z_{1}, z_{2}]} |U (z)|, \nonumber\\
    L &= \tfrac{1}{2}(z_{2} - z_{1}).
\end{align}

Note: Drazin and Reid use asterisks to represent the dimensional variables, but here we use the asterisks to indicate the dimensionless variables as is the more common convention.
...
We can now use this transformation to find dimensionless forms of the various terms of the governing equations. By the chain rule, we have
\begin{equation}
    \pdv{\mathbf{u}}{t} = \pdv{\mathbf{u}}{\mathbf{u_*}} \pdv{\mathbf{u_*}}{t_*} \pdv{t_*}{t} = \frac{V^2}{L} \pdv{\mathbf{u_*}}{t_*}.
\label{dut*}
\end{equation}

In our dimensional coordinates, $ \nabla$ is defined as
\begin{equation}
    \nabla = \pdv{x} \mathbf{i} + \pdv{y} \mathbf{j} + \pdv{z} \mathbf{k}.
\end{equation}

To transform this operator into our new coordinates we use the chain rule to get the following 
\begin{equation}
    \nabla = \pdv{x_*} \pdv{x_*}{x} \mathbf{i} + \pdv{y_*} \pdv{y_*}{y} \mathbf{j} + \pdv{z_*} \pdv{z_*}{z} \mathbf{k} = \frac{1}{L} \nabla_*.
\end{equation}

We can use this $ \nabla_*$ along with our transformation \eqref{coord_trans} to get equations for $ \nabla \mathbf{u}$, $ \nabla \cdot \mathbf{u}$, $ \Delta \mathbf{u}$ and $ \nabla p$ in their dimensionless forms as follows,
\begin{align}
    \nabla \mathbf{u} &= \frac{1}{L} \nabla_*(V \mathbf{u_*}) = \frac{V}{L} \nabla_* \mathbf{u_*}, \label{gradu*} \\
    \nabla \cdot \mathbf{u} &= \qb{\frac{1}{L} \nabla_*} \cdot (V \mathbf{u_*}) = \frac{V}{L} \nabla_* \cdot \mathbf{u_*}, \label{divu*} \\
    \Delta \mathbf{u} &= \nabla \cdot (\nabla \mathbf{u} ) = \qb{\frac{1}{L} \nabla_*} \cdot \qb{\frac{V}{L} \nabla_* \mathbf{u_*}} = \frac{V}{L^2} \Delta_* \mathbf{u_*}, \label{lapu*} \\
    \nabla p &= \qb{\frac{1}{L} \nabla_*}(\rho V^2 p_*) = \frac{\rho V^2}{L} \nabla_* p_*. \label{gradp*}
\end{align}
...
\begin{gather}
    0 = \div{\mathbf{u}} = \frac{V}{L} \nabla_* \cdot {\mathbf{u_*}}, \\
    \nabla_* \cdot {\mathbf{u_*}} = 0.
\end{gather}
...
\begin{align}
        \pdv{\mathbf{u}}{t} + \mathbf{u} \cdot \nabla \mathbf{u} &= - \frac{1}{\rho} \nabla p + \nu \Delta \mathbf{u}, \nonumber\\
        \frac{V^2}{L} \pdv{\mathbf{u_*}}{t_*} + \frac{V^2}{L} \mathbf{u_*} \cdot \nabla_* \mathbf{u_*} &= -\frac{V^2}{L} \nabla_* p_* + \frac{\nu V}{L^2} \Delta_* \mathbf{u_*}, \nonumber\\
        \pdv{\mathbf{u_*}}{t_*} + \mathbf{u_*} \cdot \nabla_* \mathbf{u_*} &= -\nabla_* p_* + \frac{\nu}{V L} \Delta_* \mathbf{u_*}.
\end{align}
Notice that $ \frac{V L}{\nu} $ is the famous \emph{Reynolds number} $ R $. Using this in the Navier-Stokes equation leaves us with our pair of \emph{dimensionless governing equations}. For simplicity, we drop the asterisks from our notation from now on as we will not deal with the dimensional case again. This means our dimensionless governing equations are now
\begin{align}
...
\begin{equation}
    \qb{\pdv{t} + U \pdv{}{x}} \mathbf{u}' + w' \frac{\text{d}U}{\text{d}z} \mathbf{i} = - \nabla p' + R^{-1} \Delta \mathbf{u}'.
\label{nslin}
\end{equation}
...
\begin{align}
    ...
    &= \qb{i(\alpha \hat{u} + \beta \hat{v}) + \frac{\text{d}\hat{w}}{\text{d}z}}\exp{i(\alpha x + \beta y - \alpha c t)}.
\end{align}
...
\begin{gather}
    \qb{\pdv{}{t} + U \pdv{}{x}} \mathbf{u}' + w' \frac{\text{d}U}{\text{d}z} \mathbf{i} = -i \alpha c \mathbf{u}' + i \alpha U \mathbf{u}' + \frac{\text{d}U}{\text{d}z} \hat{w} \mathbf{i} \nonumber\\
    = \qb{i \alpha ( U - c) \hat{\mathbf{u}} + \frac{\text{d}U}{\text{d}z} \hat{w} \mathbf{i}} \exp{i(\alpha x + \beta y - \alpha c t)},
\end{gather}
and for the right-hand side, we get
\begin{gather}
    -\nabla p' + R^{-1} \Delta \mathbf{u}' = \nonumber\\ \qb{-(i \alpha \hat{p} \mathbf{i} + i \beta \hat{p} \mathbf{j} + \dv{\hat{p}}{z} \mathbf{k}) + R^{-1} \hat{\mathbf{u}} (- \alpha^2 - \beta^2 - \dv[2]{\hat{\mathbf{u}}}{z})} \exp{i(\alpha x + \beta y - \alpha c t)}.
\end{gather}
Equating both sides and cancelling the exponential leaves us with the following 3 equations
\begin{subequations}
\begin{align}
    i \alpha (U - c) \hat{u} + \dv{U}{z} \hat{w} &= -i \alpha \hat{p} - \qb{(\alpha^2 + \beta^2)\hat{u} + \dv[2]{\hat{u}}{z}} R^{-1}, \\
    i \alpha (U - c) \hat{v} &= -i \beta \hat{p} - \qb{(\alpha^2 + \beta^2)\hat{v} + \dv[2]{\hat{v}}{z}} R^{-1}, \\
    i \alpha (U - c) \hat{w} &= \dv{\hat{p}}{z} - \qb{(\alpha^2 + \beta^2)\hat{w} + \dv[2]{\hat{w}}{z}} R^{-1}.
\end{align}   
\end{subequations}
...
These equations have the same form as equations \eqref{os1}, \eqref{oseqcomb} with $ \beta = \hat{v} = 0 $, showing that we have successfully reduced this to a 2-dimensional problem. In what follows we will use equations \eqref{os1}, \eqref{oseqcomb} with $ \beta = \hat{v} = 0$ to keep the notation simpler.
...
\end{equation}
as the exponentials cancel out (because $ \beta = 0$).
...
If we look at our boundary conditions \eqref{viscousbc} and at our stream function \eqref{streamfunction} then we can see that for $ \hat{u} = \hat{w} = 0$ to be zero on $ z = \pm 1 $  we must have the boundary conditions
\begin{equation}
    \phi(\pm 1) = \phi'(\pm 1) = 0.
    \label{osbc}
\end{equation}
